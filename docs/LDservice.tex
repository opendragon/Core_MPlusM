\ProvidesFile{LDservice.tex}[v1.0.0]
\primaryStart[TheLDOservice]{The \LDO{} Service}
The \asCode{m+mLeapDisplayOutputService} application is an Output service, displaying the
stream of values from a running \asCode{m+mLeapFingerTipsInputService} application.
...
The \asCode{m+mLeapDisplayOutputService} application is launched as a standard GUI
application.
When launched, the following dialog will be presented:
\objScaledDiagram{mpm_images/launchLeapDisplayOutputService}{launchServiceLeapDisplay}%
{Launch options for the \emph{\LDO} service}{0.8}
\condPage{}
Before the service has been started, the following will be displayed:
\objScaledDiagram{mpm_images/leapDisplayOutputServiceBeforeStart}{beforeStartLeapDisplay}%
{Display before the \emph{\LDO} service starts}{0.8}
\condPage{}
The service is started via the Operation menu:
\objScaledDiagram{mpm_images/leapDisplayOutputServiceOperationMenu}%
...
running copy of the\\
\asCode{m+mLeapFingerTipsInputService} application;
once this is done, the display will change to the following:
\objScaledDiagram{mpm_images/runningLeapDisplayOutputService}{serviceRunningLeapDisplay}%
{Display while the \emph{\LDO} service is running}{0.8}
\condPage{}
The \asCode{m+mLeapDisplayOutputService} application window will then display the
...
\objScaledDiagram{mpm_images/leapDisplayOutputServiceViewMenu}{viewMenuLeapDisplay}%
