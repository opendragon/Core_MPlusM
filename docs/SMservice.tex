...
\primaryStart[TheSMOservice]{The \SMO{} Service (\mac{} and 64\longDash{}bit \win{} only)}
...
The \emph{configure} request has two arguments \longDash{} a string value for the topic or
queue to set up on the ActiveMQ broker and an integer value for the `queue flag'.
...
The \emph{stopStreams} request terminates listening on the input stream and drops the
connection to the ActiveMQ broker.\\ 
