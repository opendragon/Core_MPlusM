...
\objScaledDiagram{mpm_images/runningAbsorberFilterService}%
...
message from the file; a value of \asCode{1} indicates no scaling while, for example, a
value of \asCode{2} indicates that the messages will be sent half as often as they were
recorded.
A value of \asCode{0} for the `loop flag' indicates no looping \longDash{} the data
contained in the file will be sent once, after the initial delay \longDash{} and a value
of \asCode{1} indicates that the data contained in the file will be sent continuously.\\
...
specified, a playback ratio of 1, an initial delay of zero seconds and a loop flag of
\asCode{1} \longDash{} indicating continuous sending of the data \longDash{} will be used.
...
Once the options are set, the following image will appear in the \emph{\MMMU} window when
the service has successfully started:
\objScaledDiagram{mpm_images/runningPlaybackFromJSONInputService}%
...
Note that the loop flag can be specified by \asBoldCode{0}, \asBoldCode{f} or
\asBoldCode{n} for sending the data exactly once and \asBoldCode{1}, \asBoldCode{t} or
\asBoldCode{y} for sending the data continuously.
