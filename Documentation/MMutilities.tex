...
The application \utilityNameX{mpmClientList} displays the clients for services that have
...
The application \utilityNameX{mpmFindServices} displays the primary channels belonging to
...
The application \utilityNameX{mpmPortLister} displays the active \yarp{} ports and
\mplusm{} entities.
...
The application \utilityNameX{mpmRequestInfo} displays information on requests for one or
...
The application \utilityNameX{mpmServiceLister} displays the active services in the
\mplusm{} installation.
...
"ServiceName": "RandomBurst", "Tag": "", "ServiceKind": "Input", "Description":
...
"d+" } ] }, { "ServicePort":  "/shorten", "ServiceName": "TruncateFloat", "Tag":
"blort", "ServiceKind": "Filter", "Description": "The Truncate Float filter
service", "ExtraInfo": "", "Requests": "", "Path": ".../mpmTruncateFloatService",
"SecondaryInputs": [ { "Name": "/shorten/input", "Protocol": "d+" } ],
"SecondaryOutputs": [ { "Name": "/shorten/output", "Protocol": "i+" } ] } ]
\end{verbatim}
\outputEnd{}
or, if tab\longDash{}delimited output is requested, with tabs shown as `\tabSymbol':
\outputBegin{}
...
/\serviceName/input/randomburst\pseudotab{}RandomBurst\pseudotwotabs{}Input\pseudotab\\
The Random Burst input service\pseudotab{}Burst period is 1, burst size is 1%
\pseudotwotabs\\
\textellipsis{}/mpmRandomBurstService\pseudotwotabs{}/\serviceName/input/randomburst/%
output\textbraceleft{}protocol=d+\textbraceright\\

/shorten\pseudotab{}TruncateFloat\pseudotab{}blort\pseudotab{}Filter\pseudotab{}The
...
The application \utilityNameX{mpmServiceMetrics} displays measurements for the channels of
...
The application \utilityNameX{mpmStopService} tells a particular active service to stop
running.\\
...
The application \utilityNameX{mpmVersion} displays the version numbers for \mplusm,
...
\tertiaryStart{\utilityNameP{mpmBlobOutputService}}
The \utilityNameX{mpmBlobOutputService} application is an Output service, accepting and
...
\item\exSp\cmdItem{c}{configure the service by proving the port number to be used}
...
\tertiaryEnd{\utilityNameE{mpmBlobOutputService}}
\condPage
...
for use by the \emph{Channel Manager} GUI application}
...
An example execution of the application, with the \utilityNameR{mpmRequestCounterService}
