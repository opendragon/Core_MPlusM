...
\item \optItem{j}{json}{generate JSON--formatted output}
\item \optItem{t}{tabs}{generate output in a tab--delimited format}
\end{itemize}
...
\item \cmdItem{f}{request the matching criteria and process it}
\item \cmdItem{q}{quit the application}
\end{itemize}
An example execution of the program, with the command--line argument \asCode{'tag!blurt'}
...
\item \optItem{h}{help}{display the list of optional parameters and arguments and leave}
\item \optItem{p}{port}{specifies the port number to be used, if a non--default port is
desired}
\item \optItem{r}{report}{report the service metrics when the application exits}
\item \optItem{t}{tag}{specifies the tag to be used as part of the service name}
\end{itemize}
...
\begin{itemize}
\item \cmdItem{?}{display a list of the available commands}
\item \cmdItem{b}{start the input stream, using the specified output file path to record
the input data}
\item \cmdItem{c}{configure the service by providing the output file path to be used}
\item \cmdItem{e}{stop the input stream, which will close the output file}
\item \cmdItem{q}{quit the application}
\item \cmdItem{r}{restart the input stream, which will cause the output file to be closed
and a new output file to be opened}
\item \cmdItem{u}{reset the configuration so that it will be reprocessed when the input
stream is restarted}
\end{itemize}
...
\utilityNameX{mpmAddressService} service, but the \emph{Channel Manager} application
...
The \utilityNameX{mpmAddressClient} application is a command--line tool to
...
\begin{itemize}
\item \makebox[4em][l]{\textbf{address}} Display just the retrieved IP address
\item \makebox[4em][l]{\textbf{both}} Display the retrieved IP address and port
\item \makebox[4em][l]{\textbf{port}} Display just the retrieved port
\item \makebox[4em][l]{\textit{other}} Use \emph{other} as the tag for the
...
\tertiaryStart{\utilityNameP{mpmBridgeService}}
The \utilityNameX{mpmBridgeService} application is a background service that is used to
route a non--\yarp{} network data source to a non--\yarp{} network data sink, via the
\utilityNameR{mpmBridgeClient} application.\\

It responds to the \requestsNameR{Miscellaneous}{Miscellaneous}{where} request sent by the
companion application \utilityNameR{mpmBridgeClient} to retrieve an IP address and port
...
\utilityNameX{mpmBridgeService} service, but the \emph{Channel Manager} application cannot
display them without a distinguishing `tag'.
If the tag is not specified, the standard name of the service will be used.
\tertiaryEnd{\utilityNameE{mpmBridgeService}}
\tertiaryStart{\utilityNameP{mpmBridgeClient}}
The \utilityNameX{mpmBridgeClient} application is a command--line tool to route a
non--\yarp{} network data source to a non--\yarp{} network data sink via the
\utilityNameR{mpmBridgeService} application.\\

It uses the \requestsNameR{Miscellaneous}{Miscellaneous}{where} request sent to the
\utilityNameR{mpmBridgeService} application to get the IP address and port to connect
to.\\
...
\utilityNameR{mpmBridgeService} application being connected to.
\tertiaryEnd{\utilityNameE{mpmBridgeClient}}
