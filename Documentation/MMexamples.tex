...
\item \cmdItem{b}{start the input stream}
\item \cmdItem{c}{configure the service; this has no effect, as the service has no
configurable parameters}
\item \cmdItem{e}{stop the input stream}
\item \cmdItem{q}{quit the application}
\item \cmdItem{r}{restart the input stream}
...
number of values sent in a group, and a `burst period' -- the number of seconds between
groups.
...
The application has two optional arguments -- the burst period, in seconds and the number
of random values to generate in each burst.
...
\begin{itemize}
\item \cmdItem{?}{display a list of the available commands}
\item \cmdItem{b}{start the output stream, sending bursts of data of the specified size
and period}
\item \cmdItem{c}{configure the service by providing the burst size and period}
\item \cmdItem{e}{stop the output stream}
\item \cmdItem{q}{quit the application}
\item \cmdItem{r}{restart the output stream}
\item \cmdItem{u}{reset the configuration so that it will be reprocessed when the output
stream is restarted}
\end{itemize}
...
\item \cmdItem{q}{quit the application}
\item \cmdItem{r}{restart the input stream, which will cause the current output file to be
...
\begin{itemize}
\item \cmdItem{?}{display a list of the available commands}
\item \cmdItem{b}{start the input and output streams}
\item \cmdItem{c}{configure the service; this has no effect, as the service has no
configurable parameters}
\item \cmdItem{e}{stop the input and output streams}
\item \cmdItem{q}{quit the application}
\item \cmdItem{r}{restart the input and output streams}
\item \cmdItem{u}{reset the configuration so that it will be reprocessed when the input
...
\begin{itemize}
\item \cmdItem{?}{display a list of the available commands}
\item \cmdItem{+}{read a number from the terminal and send it to the service via an
\requestsNameR{Examples}{Examples}{addtosum} request, to update the running sum for this
client}
\item \cmdItem{q}{send a \requestsNameR{Examples}{Examples}{stopsum} request to the
service so that it will stop calculating the running sum for this client and then exit the
application}
\item \cmdItem{r}{send a \requestsNameR{Examples}{Examples}{resetsum} request to the
service so that it will reset the running sum for this client}
\item \cmdItem{s}{send a \requestsNameR{Examples}{Examples}{startsum} request to the
