...
\newcommand{\insertTagAndEndpointDescription}[4]{For details on the usage of the `endpoint' and `tag' options, see the \emph{\WTE} appendix in the \emph{\MMMU} manual.
Once the options are set, an image similar to the following will appear in the \emph{\MMMU} window when the service has successfully started:
