...
\classNameR{Common}{InputHandler} and implements the \asCode{handleInput} method,
processing floating--point and integer data; this should be changed when developing a
custom application.
...
\classNameR{Common}{InputHandler} and invokes the `placeholder' methods of the\\
...
\classNameR{Common}{InputHandler} and simply writes out the data received by the
...
\asCode{Exemplar} to \asCode{Foo} and replace references to `exemplar' in comments with
`foo' -- note that the copyright notice at the beginning of each file includes the word
`EXEMPLARY', which should not be changed
\item In the main files for each application (\asCode{M+MFooAdapterMain.cpp},
...
\item In the files \asCode{M+MFooXXXService.h} and
...
\item In the files \asCode{M+MFooXXXRequests.h} and \asCode{M+MFooXXXService.cpp},
replace\\
...
where \asCode{XXX} is \asCode{FILTER}, \asCode{INPUT} or \asCode{OUTPUT}
\item In the file \asCode{M+MFooXXXServiceMain.cpp}, replace
...
\item If \asCode{ccmake} does not exit with success, use the `q' (quit) command and review
the \asCode{CMakeLists.txt} files to make sure that they've been edited correctly -- the
error screen from \asCode{ccmake} will indicate which files have problems
\item Execute `\asCode{cmake~.}' from the terminal session, to create the `makefiles' used
to construct the applications
\item Execute `\asCode{make}' to build the applications; errors at this point will be a
...
`\asCode{sudo~make~install}' from a terminal session in the top--level directory.
\begin{center}\textbf{On~Windows:}\end{center}
\textbf{TBD}
\tertiaryEnd{}
\secondaryEnd{}
