\ProvidesFile{MMupdatingVersionNumber.tex}[v1.0.0]
...
\begin{itemize}
\item The documentation version number is located in the \asCode{sharedSetup.tex} file in
the \asCode{Documentation} directory in the \asCode{Core\fUS{}MPlusM} directory
\longDash{} look for the `\asCode{\textbackslash{}MMV}' macro
...
number
\item\exSp{} The \emph{Channel Manager} version number is located in the \asCode{Xcode}
project settings \longDash{} make sure to change both of the \asCode{Version} and
\asCode{Build} settings in the \asCode{General} settings for the \asCode{Channel Manager}
target \longDash{} and the \asCode{CMResources.rc} file, where the `\asCode{FILEVERSION}',
`\asCode{FileVersion}' and `\asCode{ProductVersion}' values will need to be modified
...
\asCode{Utilities\fUS{}Channel Manager/Doxyfile}
\end{itemize}
