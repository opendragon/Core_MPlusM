...
\item\exSp\textbf{\secondaryRef{RegistryServiceRequests}{Registry~Service~Requests:}}
requests that are specific to the \serviceNameR[Registry~Service]{RegistryService}
...
\serviceNameR[Registry~Service]{RegistryService} is described in 
\tertiaryRef{RegistryService}{Registry~Service}.
\tertiaryStart[RegistryService]{mpmRegistryService (also known as the
...
primarily for use by the \emph{Channel Manager} application}
...
{launchServiceRegistry}{Launch options for the \serviceNameX[Registry~Service]%
{RegistryService}}{1.0}
\condPage{}
...
window when the \serviceNameX[Registry~Service]{RegistryService} has successfully started:
\objScaledDiagram{mpm_images/runningRegistryService}%
{serviceRunningRegistry}{The \emph{Channel Manager} entity for the
\serviceNameX[Registry~Service]{RegistryService}}{1.0}
...
It is used by the \emph{Channel Manager} utility to record the actual arguments used to
launch the executable containing a service.
...
\requestsNameA{Registry~Service}{RegistryService}{find}}
...
\requestsNameA{Registry~Service}{RegistryService}{remember}}
...
\requestsNameA{Registry~Service}{RegistryService}{forget}}
...
For the \examplesNameR{Services}{mpmRandomBurstInputService} application, the
configuration data consists of a floating\longDash{}point number (the burst period) and an
integer (the burst size); for the \examplesNameR{Services}{mpmRecordIntegersOutputService}
...
For the \examplesNameR{Services}{mpmRandomBurstInputService} application, starting the
streams corresponds to initiating an asynchronous random number generator; for the
\examplesNameR{Services}{mpmRecordIntegersOutputService} application, starting the streams
involves opening the service output file and for the
\examplesNameR{Services}{mpmTruncateFloatFilterService} application, starting the streams
...
\utilityNameR{mpmAddressClient} application to retrieve the IP address and port from the\\
