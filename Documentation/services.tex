...
The \serviceNameX[Service~Registry]{ServiceRegistry} application responds to a `HUP'
...
For the \examplesNameR{Services}{mpmRandomBurstService} service, the configuration
data consists of a floating--point number (the burst period) and an integer (the burst
size); for the \examplesNameR{Services}{mpmRecordIntegersService} service, the
...
For the \examplesNameR{Services}{mpmRandomBurstService} service, starting the
streams corresponds to initiating an asynchronous random number generator; for the
\examplesNameR{Services}{mpmRecordIntegersService} service, starting the streams
involves opening the service output file and for the
\examplesNameR{Services}{mpmTruncateFilterService} service, starting the streams
