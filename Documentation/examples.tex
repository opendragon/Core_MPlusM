...
The \examplesNameX{Services}{mpmRandomBurstStreamService} application is an Input service,
...
\end{itemize}
These parameters provide the initial burst period and size; if not specified, a burst
...
The \examplesNameX{Services}{mpmRecordIntegersStreamService} application is an Output
...
\end{itemize}
This parameter provides the initial output file path; if not specified, a random path in
the temporary directory will be used.
For Windows, the temporary directory being used is ``\textbackslash{}tmp'' while, on the
Mac, it will be ``/tmp''.
...
The \examplesNameX{Services}{mpmRecordIntegersStreamService} application is a Filter
