...
comments with `\asCode{foo}' -- note that the copyright notice at the beginning of each
file includes the word `\asCode{EXEMPLARY}', which should not be changed
...
contain `\asCode{Exemplar}' to contain `\asCode{Foo}' -- these will most often be the
...
\item And lastly, change the ``\asCode{Written by}'' string in the comment block at the
...
\item If \asCode{ccmake} does not exit with success, use the `\textbf{q}' (quit) command
and review the \asCode{CMakeLists.txt} files to make sure that they've been edited
correctly -- the error screen from \asCode{ccmake} will indicate which files have problems
\item Execute `\asCode{cmake~.}' from the terminal session, to create the `makefiles' used
to construct the applications
\item Execute `\asCode{make~clean~\&\&~make}' to build the applications; errors at this
