...
`\asCode{Visual Studio Community 2013}', but newer versions should also work. Note that
...
\companyReference{https://ftp.mozilla.org/pub/mozilla.org/mozilla/libraries/win32/MozillaBuildSetup-Latest.exe}%
{Mozilla};
...
\item\exSp{} Open the project file `\asCode{ogdf.sln}' with \asCode{Visual Studio},
running as Administrator
\item\exSp{} Within \asCode{Visual Studio}, select the \asCode{Debug} configuration and
build it via the menu item `\asCode{Build \bigRightArrow{} Build Solution}'
\item\exSp{} Select the \asCode{Release} configuration and build it via the menu item
...
\item\exSp{} On the command\longDash{}line, execute the command\\
`\asCode{git clone https://github.com/mozilla/gecko-dev.git \textbf{spidermonkey}}'
\item\exSp{} Execute the command `\asCode{cd~\textbf{spidermonkey}}'
...
\item\exSp{} Execute the command `\asCode{cd~\textbf{spidermonkey}}'
\item\exSp{} Execute the command `\asCode{cd~js/src}'
\item\exSp{} Execute the command `\asCode{/usr/local/bin/autoconf-2.13}'
\item\exSp{} Execute the command `\asCode{mkdir~\fUS{}obj\fUS}'
\item\exSp{} Execute the command `\asCode{cd~\fUS{}obj\fUS}'
\item\exSp{} Execute the command\\
`\asCode{../configure {-}{-}enable-release {-}{-}enable-more-deterministic {-}{-}enable-build-nspr}'
\item\exSp{} Execute the command `\asCode{mozmake}'
\item\exSp{} Execute the command `\asCode{mkdir \textbf{devel}/javascript}', where
\textbf{devel} is the directory containing the \mplusm{} source code
\item\exSp{} Execute the command `\asCode{mkdir \textbf{devel}/javascript/bin}'
\item\exSp{} Execute the command `\asCode{mkdir \textbf{devel}/javascript/include}'
\item\exSp{} Execute the command `\asCode{mkdir \textbf{devel}/javascript/lib}'
\item\exSp{} Execute the command `\asCode{mkdir \textbf{devel}/javascript/nspr-include}'
\item\exSp{} Execute the command `\asCode{cd dist}'
\item\exSp{} Execute the command `\asCode{cp bin/*.dll \textbf{devel}/javascript/bin}'
\item\exSp{} Execute the command `\asCode{cp lib/* \textbf{devel}/javascript/lib}'
\item\exSp{} Execute the command `\asCode{cp -R include/* \textbf{devel}/javascript/include}'
\item\exSp{} Execute the command `\asCode{cp -R nspr-include/* \textbf{devel}/javascript/nspr-include}'
...
\item\exSp{} Exit from the \asCode{CMake} GUI application
\item In Explorer, open the directory containing the \mplusm{} source code, \asCode{devel}
\item\exSp{} Open the directory `\asCode{Core\fUS{}MPlusM}' within \asCode{devel}
\item\exSp{} Open the project file `\asCode{M+M.sln}' with \asCode{Visual Studio}, running
as Administrator
\item\exSp{} Within \asCode{Visual Studio}, select the \asCode{Debug} configuration and
build it via the menu item `\asCode{Build \bigRightArrow{} Build Solution}'
\item\exSp{} Select the \asCode{Release} configuration and build it via the menu item
...
\item\exSp{} Select the \asCode{INSTALL} target and install the \asCode{Release} files via
the menu item `\asCode{Build \bigRightArrow{} Project Only \bigRightArrow{} Build Only
INSTALL}'; note that this will fail the first time, since it attempts to install Channel
Manager, which has not been built yet
\item\exSp{} Exit from \asCode{Visual Studio}
