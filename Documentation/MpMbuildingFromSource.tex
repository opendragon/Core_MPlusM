...
\asCode{uuid}, \asCode{sqlite}, \asCode{X11}, \asCode{xft}, \asCode{X11} extensions and
\asCode{libtool} packages
\end{itemize}
The prerequisites can be obtained via the following commands in the \asCode{Terminal}
application:
\begin{itemize}
\item \asCode{apt-get install cmake-curses-gui cmake git g++ clang autoconf2.13 python2.7
wget}
\item\exSp\asCode{apt-get install libtool libavahi-compat-libdnssd-dev libavahi-core-dev
uuid-dev}
\item\exSp\asCode{apt-get install sqlite e2fsprogs e2fslibs-dev libicu-dev libssl-dev}
\item\exSp\asCode{apt-get install libx11-dev libxft-dev libxrandr-dev libxinerama-dev
libxcursor-dev}
\end{itemize}
...
code \longDash{} \asCode{\textbf{devel}/yarp} or\\
\asCode{\textbf{devel}/yarp\fUS{}MPlusM}
\item\exSp{}Execute the following commands:
\begin{itemize}
...
\item\exSp\asCode{make clean \&\& make}
\item\exSp\asCode{sudo make install}
\item\exSp\asCode{sudo cp dist/bin/libmozglue.so /opt/m+m/spidermonkey/lib}
%\item\exSp\asCode{sudo install\fUS{}name\fUS{}tool -change
%"@executable\fUS{}path/libmozglue.dylib" \textbackslash\\
%"@rpath/libmozglue.dylib" /opt/m+m/spidermonkey/lib/libmozjs-\spiderversion.dylib}; note
%the single dash for `\asCode{-change}'
\end{itemize}
%\item\exSp{}In the \asCode{Terminal} application, move to the directory containing the \mplusm{}
%source code, \asCode{\textbf{devel}}
%\item\exSp{}Execute the command `\asCode{sudo Core\fUS{}MPlusM/replace-links.sh
%\textbackslash\\
%/opt/m+m/spidermonkey/include/mozjs-\spiderversion}'; this replaces symbolic links in the
