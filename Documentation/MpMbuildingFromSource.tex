...
Macintosh OS X) of \asCode{Optional} component (on Microsoft Windows).
...
For example, the JavaScript Input / Output service uses the \textbf{SpiderMonkey} engine
to support \compLang{JavaScript}-based services, but only the minimum files needed for it
are installed by default \longDash{} in order to build the JavaScript Input / Output
service, the header files for \textbf{SpiderMonkey}, at least, must be installed.
...
\textbf{SpiderMonkey} is the \compLang{JavaScript} engine used with the JavaScript Input /
Output service of \mplusm{}; the files needed to compile and link with
\textbf{SpiderMonkey} are installed as part of the normal \mplusm{} installation
procedure, so it's not necessary to rebuild \textbf{SpiderMonkey}.
...
\item\exSp{}Select the `\asCode{activemq-cpp}' row in the left-hand table and open the
\asCode{activemq-cpp Property Pages} panel via the menu item
...
without quotes
\item\exSp{}Click on the \asCode{OK} button on the bottom-right of the panel
\item\exSp{}Select the \asCode{Debug} :\ \asCode{x64} configuration and build it via
the menu item `\asCode{Build \bigRightArrow{} Build Solution}'
\item\exSp{}Select the \asCode{Release} :\ \asCode{x64} configuration and build it via
the menu item `\asCode{Build \bigRightArrow{} Build Solution}'
\item\exSp{}Exit from \asCode{Visual Studio}
