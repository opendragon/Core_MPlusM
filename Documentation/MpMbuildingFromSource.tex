...
Latest.exe}
\item\exSp{}Select a directory to hold the \textbf{SpiderMonkey} source code, which will be
referred to as `\asCode{spidermonkey}'; this can be a sister directory to the
\asCode{\textbf{devel}} directory, but it need not be
\item\exSp{}Open a Command Prompt window, running as Administrator
\item\exSp{}Execute the command
`\asCode{git clone https://github.com/mozilla/gecko-dev.git \textbf{spidermonkey}}', if
\asCode{git} is available on the system, or use \asCode{SourceTree} to clone the
repository\\
...
`\asCode{git clone https://git-wip-us.apache.org/repos/asf/activemq-cpp.git
\textbf{ActiveMQ}}', if \asCode{git} is available on the system, or use
\asCode{SourceTree} to clone the repository\\
